...
- 사업자번호 최대 **10,000건 동시 조회** (100건 단위 자동 분할)
- 계속사업자 / 휴업자 / 폐업자 상태 확인
- 과세유형 및 폐업일 조회
- `.txt` / `.csv` / `.xlsx` **파일 업로드** 및 **드래그앤드롭** 지원
- 결과 **CSV 다운로드**
- 상태별 필터 및 통계 요약
- **예시 이미지** 서버 저장 (관리자만 추가, 누구나 열람)
...
예시 이미지는 `public/examples/` 폴더에서 관리합니다.

### 이미지 추가 방법

1. `public/examples/` 폴더에 이미지 파일 넣기  
   (예: `sample1.png`, `excel_example.jpg`)

2. `public/examples/manifest.json` 파일에 추가:
...
3. GitHub에 push → Vercel 자동 재배포 → 사이트에 즉시 반영

### 이미지 삭제 방법

1. `manifest.json`에서 해당 항목 제거
2. `public/examples/`에서 파일 삭제 (선택)
3. GitHub에 push

> **참고:** `manifest.json`이 없거나 비어있으면 예시 이미지 섹션이 자동으로 숨겨집니다.
...
│   ├── index.js         # 메인 UI
│   └── api/
│       └── check.js     # 국세청 API 프록시 (자동 분할)
├── public/
│   └── examples/
│       ├── manifest.json # 예시 이미지 목록 설정
│       ├── sample1.png   # 예시 이미지 파일들
│       └── sample2.png
├── styles/
│   └── globals.css      # 전체 스타일
├── .env.example         # 환경변수 예시
